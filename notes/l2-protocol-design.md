...
- The deployer's key is permanently bound to the token as its sole **SSS** (Single Signer Server).
...
- **Soft finality**: instant, via SSS signature, backed by an on-chain bond with cryptographic slashing on equivocation.
...
GenesisCommitment {
    ...
    bond_outpoint:   Bitcoin UTXO holding the SSS bond,
    metadata: {
...
- The deployer's pubkey is the **only authorized SSS** for state transitions of this asset.
...
### Why this resolves the multi-SSS fork problem

In the earlier discussion we hit a wall: free per-receiver SSS choice forks the network state. The fix here is structural:

- Each asset has exactly one SSS (its issuer).
- All transactions on an asset must be SSS-signed to be considered for soft-finality.
- Receivers don't choose SSS per transaction — they choose whether they trust the **issuer of the asset they are receiving**.
...
The detailed conflict-resolution rules — including how unsigned and SSS-signed transactions interact at different confirmation depths — are specified in [`sss-trust-model.md`](sss-trust-model.md).
...
Each asset has an independent state tree. The SSS maintains:
...
The SSS is **stateless from the protocol's perspective in one important sense**: anyone can reconstruct `AssetState` deterministically from Bitcoin history alone. The SSS is a convenience and a soft-finality oracle, not a custodian of the asset's truth.
...
1. Alice's wallet:
   ...
   - Sends to SSS of asset X: { proof, new_state_hash, nullifier, out_coin_for_bob }
...
4. Bob's wallet verifies:
   - SP1 proof valid ✓
   - SSS signature valid against asset's Genesis pubkey ✓
...
- SSS compute (proof generation), realistic ~100–1000 TPS per SSS
...
Two finality levels, both economically/cryptographically grounded. For the complete validity hierarchy and the four receiver trust profiles, see [`sss-trust-model.md`](sss-trust-model.md).

### 4.1 Soft Finality — SSS-Attested

A transaction is **soft-final** when:
- SSS signature over the transaction exists
- SSS's bond is sufficient to cover the transaction value

**Security argument**: the SSS cannot equivocate (sign two conflicting transitions of Alice's account) without producing a cryptographic slashing proof — two Schnorr signatures over different messages under the same key, fully verifiable on Bitcoin via Taproot script.
...
For receivers assuming an active SSS+miner conspiracy, the relevant threshold is 12 confirmations on an unsigned transaction (see Profile 4 in [`sss-trust-model.md`](sss-trust-model.md)).

### 4.3 Recovery from soft-final failures

If a soft-final transaction is later contradicted by hard-final state (e.g. miner bribery successfully reorders the inscription stream against the SSS's attestation), the receiver's recourse is:

1. Submit slashing proof to Bitcoin: shows SSS-signed transaction + conflicting canonical Bitcoin state.
2. Bond payout to victim per the bond contract's distribution rules.

The SSS's bond serves as economic insurance against any failure of soft-finality.
...
The SSS bond is a Taproot UTXO with two spending paths:

```
Path A — Normal withdrawal (SSS voluntary close):
    Conditions:
    - SSS signature
    - Timelock: 90 days from last attestation involving this bond
    - Purpose: allows the SSS to retire an asset cleanly

Path B — Slashing (Equivocation proof):
    Conditions:
    - Schnorr signature 1 by SSS over message m1
    - Schnorr signature 2 by SSS over message m2
    - m1 ≠ m2
...
bond_size > max(over any rolling window W):
    ...
    if the SSS equivocates
```

Practically, this means:
- Per-transaction soft-final cap: `bond_size / safety_factor`
- The SSS publishes its bond status (current balance, pending claims) so wallets can compute exposure
...
**Pro-rata claim window**: First slashing proof submitted on-chain opens a claim window (e.g., 144 blocks ≈ 1 day). All affected receivers can register claims by submitting SSS-signed proofs of their unfulfilled transactions. After the window, the bond is distributed pro-rata.

Trade-offs:
- Pro-rata is fair but complex (multi-step claim flow).
- First-come-first-serve is simpler but encourages racing.

Recommendation: pro-rata for stablecoins / high-value assets, first-come for low-stakes assets where SSS equivocation is unlikely to be coordinated.
...
Bond drainage by repeated micro-slashing must be detected and the SSS paused. Wallets that observe ongoing slashing claims should require additional confirmations or refuse soft-finality entirely.

---

## 6. Multi-Token Support

The protocol scales naturally to arbitrary tokens because each asset has an isolated state tree and SSS.
...
A swap between two assets (e.g. zkBTC ↔ stablecoin) requires both SSSs to coordinate. Two natural approaches:

**Atomic swap via HTLC analog**: standard cross-chain swap construction, using ZK-hashlocks for privacy. Each SSS signs only its asset's leg.

**Cross-asset venue**: a separate SSS operates a swap venue (third asset issuance: the venue's LP shares). All swaps go through that venue, which itself bonds. This is the building block for DEX-style trading.

The perpetuals design ([`perpetuals-design.md`](perpetuals-design.md)) builds on the cross-asset venue pattern.

### 6.4 Privacy modes

The issuer chooses the asset's privacy mode at genesis:

- **Shielded**: amounts, ownership, history all hidden. The SSS sees state, no one else. Standard zkCoins privacy.
...
A core property of any L2 claim is: **what happens if the SSS disappears?**

### 7.1 Force-exit mechanism

A holder can unilaterally exit an asset back to Bitcoin (or to a separate SSS, if the asset has a cross-SSS escape) without the original SSS's cooperation.

```
ForceExitClaim {
    holder_account_proof:  ZK proof that holder owns state X in asset Y
    last_known_anchor:     Bitcoin block height of last SSS batch
    challenge_period:      e.g., 144 blocks (1 day) for the SSS to refute
}
```

Process:
1. Holder posts ForceExitClaim as Bitcoin inscription.
2. The SSS has `challenge_period` blocks to refute by showing either:
...
### 7.2 SSS offline scenarios

If the SSS is offline:
...
If the SSS is permanently dead:
- All holders force-exit within one challenge period.
- The asset is effectively wound down.

This makes the protocol resilient to SSS failure: the worst-case outcome is loss of soft-finality, not loss of funds.

### 7.3 SSS transfer

The SSS role can be voluntarily transferred via a Bitcoin inscription signed by the current SSS. This allows:
- Sale of an asset's SSS role
- Migration to a federated SSS (M-of-N multisig as the SSS key)
- Emergency handoff if the SSS operator wants to retire
...
| Datum | Visible to SSS | Visible to Counterparties | Visible Publicly |
|---|:-:|:-:|:-:|
...
| Total supply | yes | yes (SSS publishes) | yes |
...
- The SSS's identity (it's the issuer, public from genesis).

### 8.3 Privacy from the SSS

This is the residual trust point. The SSS sees everything for accounts under its asset. Mitigations:

- **Blind-signed attestations** (research): receiver gets the SSS signature without the SSS learning the transfer details. Cryptographically possible, not trivial.
- **Multiple SSSs per asset class** (federation): no single SSS sees all transactions of a given user. Re-introduces the cross-SSS coordination problem.
- **Asset segmentation**: a user can spread holdings across multiple assets with different SSSs, fragmenting the per-SSS view.

For most use cases, accepting that the SSS knows your activity is comparable to accepting that your bank knows your activity. The key win is that **nobody else** does.
...
3. **Force-exit UX**: how does a holder build a ForceExitClaim without the SSS's data? Requires the holder to keep their own state proofs continuously up to date. Wallet UX challenge.

4. **SSS rotation atomicity**: ensuring no transactions are lost or duplicated during an SSS key handover. Probably needs a coordinated handoff inscription pattern.
...
6. **Blind attestation**: cryptographic construction that lets the SSS attest without seeing transaction content. Research direction.

7. **DA for SSS batches**: currently the SSS publishes the full batch data inscribed. For high-volume assets this may exceed inscription size limits. Need batched-batch aggregation or off-chain DA with on-chain hashes only.
...
2. **Implement reference SSS** as an extension of the existing ZeroSync ZKCoins server: add asset metadata to genesis, multi-asset state trees, attestation signing endpoint, bond status reporting.
...
| **SSS** (Single Signer Server) | The single key authorized to attest state transitions for an asset (equals issuer at genesis) |
| **Attestation** | The SSS's Schnorr signature on a transaction, used for soft-finality |
| **Bond** | Bitcoin UTXO held by the SSS, slashable on equivocation |
| **Soft finality** | Acceptance based on SSS attestation + bond coverage |
| **Hard finality** | Acceptance based on 6 Bitcoin confirmations of the relevant nullifier |
| **Force-exit** | Unilateral withdrawal by a holder without SSS cooperation |
| **Nullifier** | The 64-byte on-chain commitment that proves a transition occurred |

For the complete glossary including trust profile terminology, see [`sss-trust-model.md`](sss-trust-model.md).
...
2. **SSS role**: Shielded CSV has no formal SSS — each node validates independently. This design introduces a privileged SSS per asset for soft-finality, while preserving the independent-validation property for hard-finality.
