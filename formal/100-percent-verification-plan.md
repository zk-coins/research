...
**Owner:** project lead
**Spec baseline:** [`zk-coins/docs@ed7fdece`](https://github.com/zk-coins/docs/commit/ed7fdece) (spec-v1.1) — current `develop` tip. Baseline progression: `a7a9f97` (pre-redesign, Pass-3 audit oracle) → `b6972b8` (spec-v1.0; + [`docs#40`](https://github.com/zk-coins/docs/pull/40) Variant-2 constant-per-batch on-chain redesign, **merged 2026-06-06**, per decision **D1**) → `ed7fdece` (spec-v1.1; + [`docs#46`](https://github.com/zk-coins/docs/pull/46) deployment topology, [`docs#47`](https://github.com/zk-coins/docs/pull/47) member_root locator + binary `C_batch` + §3.8 fee-coin + §4.2.1 ZBE, [`docs#48`](https://github.com/zk-coins/docs/pull/48) no spec change; advanced per decision **D4**). The `a7a9f97` pin and the Pass-3 audit predate `docs#40`; §3-dependent properties (P2, P5, P6) were reconciled against the on-chain layer in Phase 4. The `b6972b8` → `ed7fdece` (#47) re-verification is **diff-confirm**: every existing property stays valid because the models abstract exactly what #47 concretized, and three new load-bearing invariants cover the genuinely new #47 guarantees (see Phase-5 / §Dependencies D4).
...
- **D4 — spec-v1.1 baseline bump (resolved 2026-06-07):** advance the baseline from `b6972b8` (spec-v1.0) to `ed7fdece` (spec-v1.1 = + `docs#46`/`#47`/`#48`). A clause-by-clause analysis concluded the bump is **diff-confirm**: every P01–P10 check stays valid (the TLA+ models abstract exactly what #47 concretized — `bundle_locator` is an opaque injective Int tag, roots are sets under A16, encryption is "who holds the key" under A8–A11, digests are structured collision-free values). `docs#46` is verification-neutral (deployment topology + a documented A13 trust trade-off; one-line P06 mention). `docs#48` did not touch specification.md. `docs#47` introduces three genuinely new guarantees that move from hand-argument to load-bearing machine-checked invariants: (1) the member_root locator binds member SET **and** ORDER (P02 `member_root.tla`); (2) §3.8 fee-coin atomicity under the single `ocr` (P03 `fee_atomicity.tla`); (3) §4.2.1 ZBE chunk (N,i)-AAD anti-truncation (P08 `zbe.tla`). Each is unbounded-inductive with a runnable negative control.
