...
| `RECURSIVE_ACCUMULATOR_CHECKPOINT.md` | new (concept, 2026-07-11) | In-house cure for the DA **retention** wall (`R-D7-3`/`P17`): a rolling recursive PCD proof of accumulator progress + self-verifying nullifier snapshot lets the network discard pre-checkpoint `BatchBundle`s. Keeps constant on-chain size; no aux chain, no token, no trusted setup. Alternative to `ACCUMULATOR_SELF_PUBLISH.md`. |
